...
tp.ChangeMonitor.prototype = {
    syncModel: function syncModel(target, model) {
...
(function () {
    ...
    tp.formatDate = function (date, format) {
        ...
        format = format.replace('yyyy', y).replace('yy', y.toString().right(2)).replace('ss', s.toString().padLeft(2, '0')).replace('s', s).replace('f', f).replace('mm', n.toString().padLeft(2, '0')).replace('m', n).replace('HH', h.toString().padLeft(2, '0')).replace('H', h).replace('hh', (h > 12 ? h - 12 : h).toString().padLeft(2, '0')).replace('h', h > 12 ? h - 12 : h)
        //replace conflicting tokens with alternate tokens
        .replace('tt', h > 11 ? '>>' : '<<').replace('t', h > 11 ? '##' : '$$').replace('MMMM', '!!').replace('MMM', '@@').replace('MM', m.toString().padLeft(2, '0')).replace('M', m).replace('dddd', '^^').replace('ddd', '&&').replace('dd', d.toString().padLeft(2, '0')).replace('d', d)
        //replace alternate tokens
        .replace('>>', 'PM').replace('<<', 'AM').replace('##', 'P').replace('$$', 'A').replace('!!', monthNames[m - 1]).replace('@@', monthNames[m - 1].substring(0, 3)).replace('^^', dayNames[w]).replace('&&', dayNames[w].substring(0, 3));
    ...
    tp.on = function (elem, event, targetSelector, listener) {
        ...
        var privateListener = function privateListener(evt) {
...
tp.Http.prototype = {
    serialize: function serialize(obj, props) {
    ...
    createXhr: function createXhr(type, url, callback, error) {
...
tp.ServerPager.prototype = {
    get: function get(model, callback, error) {
        var self = this;
        var h = new tp.Http();
    ...
    copyProps: function copyProps(from, to) {
...
tp.ClientPager.prototype = {
    getSkip: function getSkip(model) {
        var data = model;
        if (data.PageCount == 0) {
            return 0;
        }
    ...
    get: function get(model, callback, error) {
        ...
        //gryst.logging = true;
        var count,
            qry = gryst.from(this.data);
        if (tp.isNullOrEmpty(model.Search) === false) {
            var props = gryst.from(model.Columns).where(function (c) {
                return c !== undefined;
            }).select('Field').run();
...
tp.PagingModel = function (data) {
    ...
    Object.defineProperty(self, 'Skip', {
        get: function get() {
            ...
            if (self.Page < 1) self.Page = 1;else if (self.Page > self.PageCount) return self.Page = self.PageCount;
...
(function () {
    ...
    var extend = function extend(name, func) {
    ...
    var callbind = FP.bind ? FP.bind.bind(FP.call) : (function (call) {
...
tp.Table.resolveTypes = function (config) {
    config.Columns.forEach(function (col) {
        for (var i = 0; i < config.data.length; i++) {
            if (data[i][col.Field] !== null) {
                col.Type = typeof data[i][col.Field];
                break;
            }
        }
    });
...
tp.Table.fixDates = function (data, typeMetadata) {
    var type,
        props = Object.getOwnPropertyNames(typeMetadata);
    props.forEach(function (prop) {
